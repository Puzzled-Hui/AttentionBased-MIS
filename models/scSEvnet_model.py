...
class scSEVnetModel(BaseModel):
    """This class implements the scSEVnetModel model, for 3D image-segmentation"""
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        # ============================== By ZMH 0325 modify ==============================
        self.loss_names = ['scSEVnet_final_output']
        ...
        # ============================== By ZMH 0325 modify ==============================
        self.model_names= ['scSEVnet']

        # =============================== Define Network =================================
        # =======      define the network structure, implemented in networks.py      =====
        # ============================== By ZMH 0325 modify ==============================
        self.netscSEVnet = networks.define_scSEVnet(in_channels=opt.in_channels,
        ...
        if self.isTrain:
            if(opt.loss_mode == 'CE'):
                self.criterionscSEVnet_final_output = nn.CrossEntropyLoss()
            if(opt.loss_mode == 'DICE'):
                self.criterionscSEVnet_final_output = losses.DiceLoss(ignore_index=opt.ignore_index,sigmoid_normalization=opt.sigmoid_normalization)
            if (opt.loss_mode == 'CBL'):
                self.criterionscSEVnet_final_output = losses.CombinedLoss(ignore_index=opt.ignore_index,weight=self.init_weight,sigmoid_normalization=opt.sigmoid_normalization)
            if (opt.loss_mode == 'GDL'):
                self.criterionscSEVnet_final_output = losses.GeneralizedDiceLoss(ignore_index=opt.ignore_index,sigmoid_normalization=opt.sigmoid_normalization)

            if(opt.optim_mode == 'Adam'):
                self.optimizer_scSEvnet = torch.optim.Adam(params=self.netscSEVnet.parameters(),lr=opt.init_learningrate)
            if(opt.optim_mode == 'SGD'):
                self.optimizer_scSEvnet = torch.optim.SGD(params=self.netscSEVnet.parameters())
            self.optimizers.append(self.optimizer_scSEvnet)
        #if use apex
        #self.netscSEVnet,self.optimizer_vnet = amp.initialize(self.netscSEVnet, self.optimizer_vnet, opt_level="O1")
    ...
    def forward(self):
        ...
        self.Prediction = self.netscSEVnet(self.Image)  # netscSEVnet(Image)
        ...
        if (self.loss_mode == 'CE'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    Mask += self.Mask[:, i,...] * i
            self.loss_scSEVnet_final_output = self.criterionscSEVnet_final_output(self.Prediction,Mask.long())
        if (self.loss_mode == 'DICE' or self.loss_mode == 'GDL'):
            self.loss_scSEVnet_final_output = self.criterionscSEVnet_final_output(self.Prediction, self.Mask)
        if (self.loss_mode == 'CBL'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    CEMask += self.Mask[:, i,...] * i
            self.loss_scSEVnet_final_output = self.criterionscSEVnet_final_output(self.Prediction, self.Mask, CEMask,self.flag,coeff_weight=self.coeff_weight)
        # if use apex
        # with amp.scale_loss(self.loss_scSEVnet_final_output, self.optimizer_vnet) as scaled_loss:
            # scaled_loss.backward()
        self.loss_scSEVnet_final_output.backward()

    def optimize_parameters(self):
        self.forward()
        self.optimizer_scSEvnet.zero_grad()
        self.backward()
        self.optimizer_scSEvnet.step()
