...
class PEUnet3dModel(BaseModel):
    """
    This class implements the PEUnet3d model, for 3D image-segmentation
    """
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        """
        parser.add_argument('--channel_hierarchy',type=int,default=2,help='how many hierarchies do an inter-middlestage feature map should have')
        return parser
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['PEUnet3d_final_output']
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        self.model_names= ['PEUnet3d']

        # define networks
        self.netPEUnet3d = networks.define_PEUnet3d(in_channels=opt.in_channels,
        ...
        if self.isTrain:
            ...
            if(opt.loss_mode == 'CE'):
                self.criterionPEUnet3d_final_output = nn.CrossEntropyLoss()
            if(opt.loss_mode == 'FC'):
                self.criterionPEUnet3d_final_output = losses.FocalLoss()
            if(opt.loss_mode == 'DICE'):
                #这里需要补一个index的opt参数
                self.criterionPEUnet3d_final_output = losses.DiceLoss(ignore_index=opt.ignore_index,sigmoid_normalization=opt.sigmoid_normalization)
            if (opt.loss_mode == 'CBL'):
                self.criterionPEUnet3d_final_output = losses.CombinedLoss(ignore_index=opt.ignore_index,weight=self.init_weight,sigmoid_normalization=opt.sigmoid_normalization)
                #self.criterionPEUnet3d_final_output = losses.CE_Dynamicweighted(ignore_index=opt.ignore_index,weight=self.init_weight,sigmoid_normalization=opt.sigmoid_normalization)                
            if (opt.loss_mode == 'GDL'):
                self.criterionPEUnet3d_final_output = losses.GeneralizedDiceLoss(ignore_index=opt.ignore_index,sigmoid_normalization=opt.sigmoid_normalization)
            # initialize optimizers; schedulers will be automatically created by function <BaseModel.setup>.
            if(opt.optim_mode == 'Adam'):
                self.optimizer_PEunet3d = torch.optim.Adam(params=self.netPEUnet3d.parameters(),lr=opt.init_learningrate)
            if(opt.optim_mode == 'SGD'):
                self.optimizer_PEunet3d = torch.optim.SGD(params=self.netPEUnet3d.parameters())
            # append optimizers;schedulers will be automatically printed by function<BaseModel.update_learning_rate>
            self.optimizers.append(self.optimizer_PEunet3d)
    ...
    def forward(self):
        ...
        self.Prediction = self.netPEUnet3d(self.Image)  # netPEUnet3d(Image)
        ...
        if (self.loss_mode == 'CE' or self.loss_mode == 'FC'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    Mask += self.Mask[:, i,...] * i
            self.loss_PEUnet3d_final_output = self.criterionPEUnet3d_final_output(self.Prediction,Mask.long())
        if (self.loss_mode == 'DICE' or self.loss_mode == 'GDL'):
            self.loss_PEUnet3d_final_output = self.criterionPEUnet3d_final_output(self.Prediction, self.Mask)
        if (self.loss_mode == 'CBL'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    CEMask += self.Mask[:, i,...] * i
            self.loss_PEUnet3d_final_output = self.criterionPEUnet3d_final_output(self.Prediction, self.Mask, CEMask,self.flag,coeff_weight=self.coeff_weight)
        self.loss_PEUnet3d_final_output.backward()

    def optimize_parameters(self):
        self.forward()
        self.optimizer_PEunet3d.zero_grad()
        self.backward()
        self.optimizer_PEunet3d.step()
