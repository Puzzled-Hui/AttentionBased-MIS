...
class csSEunet3dModel(BaseModel):
    """
    This class implements the csA 3D UNet model, for 3D image-segmentation
    """
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['csSE_Unet3d_final_output']
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        self.model_names= ['csSE_Unet3d']

        # define networks correspond
        self.netcsSE_Unet3d = networks.define_csSE_Unet3d(in_channels=opt.in_channels,
        ...
        if self.isTrain:
            ...
            if(opt.loss_mode == 'CE'):
                self.criterioncsSE_Unet3d_final_output = nn.CrossEntropyLoss()
            if(opt.loss_mode == 'DICE'):
                self.criterioncsSE_Unet3d_final_output = losses.DiceLoss(ignore_index=opt.ignore_index,sigmoid_normalization=opt.sigmoid_normalization)
            if (opt.loss_mode == 'CBL'):
                self.criterioncsSE_Unet3d_final_output = losses.CombinedLoss(ignore_index=opt.ignore_index,weight=self.init_weight,sigmoid_normalization=opt.sigmoid_normalization)
            # initialize optimizers; schedulers will be automatically created by function <BaseModel.setup>.
            if(opt.optim_mode == 'Adam'):
                self.optimizer_csSE_unet3d = torch.optim.Adam(params=self.netcsSE_Unet3d.parameters(),lr=opt.init_learningrate)
            if(opt.optim_mode == 'SGD'):
                self.optimizer_csSE_unet3d = torch.optim.SGD(params=self.netcsSE_Unet3d.parameters())
            # append optimizers;schedulers will be automatically printed by function<BaseModel.update_learning_rate>
            self.optimizers.append(self.optimizer_csSE_unet3d)
    ...
    def forward(self):
        ...
        self.Prediction = self.netcsSE_Unet3d(self.Image)  # netcsSE_Unet3d(Image)
        ...
        if (self.loss_mode == 'CE'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    Mask += self.Mask[:, i,...] * i
            self.loss_csSE_Unet3d_final_output = self.criterioncsSE_Unet3d_final_output(self.Prediction,Mask.long())
        if (self.loss_mode == 'DICE'):
            self.loss_csSE_Unet3d_final_output = self.criterioncsSE_Unet3d_final_output(self.Prediction, self.Mask)
        if (self.loss_mode == 'CBL'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    CEMask += self.Mask[:, i,...] * i
            self.loss_csSE_Unet3d_final_output = self.criterioncsSE_Unet3d_final_output(self.Prediction, self.Mask, CEMask,self.flag,coeff_weight=self.coeff_weight)
        self.loss_csSE_Unet3d_final_output.backward()

    def optimize_parameters(self):
        self.forward()
        self.optimizer_csSE_unet3d.zero_grad()
        self.backward()
        self.optimizer_csSE_unet3d.step()
