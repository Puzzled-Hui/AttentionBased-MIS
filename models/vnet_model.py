...
class VnetModel(BaseModel):
    """
    This class implements the VNet model, for 3D image-segmentation
    """
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['Vnet_final_output']
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        self.model_names= ['Vnet']

        # define networks
        self.netVnet = networks.define_Vnet(in_channels=opt.in_channels,
        ...
        if self.isTrain:
            ...
            if(opt.loss_mode == 'CE'):
                self.criterionVnet_final_output = nn.CrossEntropyLoss()
            if(opt.loss_mode == 'DICE'):
                #这里需要补一个index的opt参数
                self.criterionVnet_final_output = losses.DiceLoss(ignore_index=opt.ignore_index,sigmoid_normalization=opt.sigmoid_normalization)
            if (opt.loss_mode == 'CBL'):
                self.criterionVnet_final_output = losses.CombinedLoss(ignore_index=opt.ignore_index,weight=self.init_weight,sigmoid_normalization=opt.sigmoid_normalization)
            if (opt.loss_mode == 'GDL'):
                self.criterionVnet_final_output = losses.GeneralizedDiceLoss(ignore_index=opt.ignore_index,sigmoid_normalization=opt.sigmoid_normalization)
            # initialize optimizers; schedulers will be automatically created by function <BaseModel.setup>.
            if(opt.optim_mode == 'Adam'):
                self.optimizer_vnet = torch.optim.Adam(params=self.netVnet.parameters(),lr=opt.init_learningrate)
            if(opt.optim_mode == 'SGD'):
                self.optimizer_vnet = torch.optim.SGD(params=self.netVnet.parameters())
        ...
        #self.netVnet,self.optimizer_vnet = amp.initialize(self.netVnet, self.optimizer_vnet, opt_level="O1")
    ...
    def forward(self):
        ...
        self.Prediction = self.netVnet(self.Image)  # netVnet(Image)
        ...
        if (self.loss_mode == 'CE'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    Mask += self.Mask[:, i,...] * i
            self.loss_Vnet_final_output = self.criterionVnet_final_output(self.Prediction,Mask.long())
        if (self.loss_mode == 'DICE' or self.loss_mode == 'GDL'):
            self.loss_Vnet_final_output = self.criterionVnet_final_output(self.Prediction, self.Mask)
        if (self.loss_mode == 'CBL'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    CEMask += self.Mask[:, i,...] * i
            self.loss_Vnet_final_output = self.criterionVnet_final_output(self.Prediction, self.Mask, CEMask,self.flag,coeff_weight=self.coeff_weight)
        #with amp.scale_loss(self.loss_Vnet_final_output, self.optimizer_vnet) as scaled_loss:
            #scaled_loss.backward()
        self.loss_Vnet_final_output.backward()
