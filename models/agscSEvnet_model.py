...
class agscSEVnetModel(BaseModel):
    """This class implements the PAVNet model, for 3D image-segmentation"""
    @staticmethod
    ...
    def __init__(self, opt):
        ...
        # ============================== By ZMH 0325 modify ==============================
        self.loss_names = ['agscSEVnet_final_output']
        ...
        # ============================== By ZMH 0325 modify ==============================
        self.model_names= ['agscSEVnet']

        # =============================== Define Network =================================
        # =======      define the network structure, implemented in networks.py      =====
        # ============================== By ZMH 0325 modify ==============================
        self.netagscSEVnet = networks.define_agscSEVnet(in_channels=opt.in_channels,
        ...
        if self.isTrain:
            if(opt.loss_mode == 'CE'):
                self.criterionagscSEVnet_final_output = nn.CrossEntropyLoss()
            if(opt.loss_mode == 'DICE'):
                self.criterionagscSEVnet_final_output = losses.DiceLoss(ignore_index=opt.ignore_index,sigmoid_normalization=opt.sigmoid_normalization)
            if (opt.loss_mode == 'CBL'):
                self.criterionagscSEVnet_final_output = losses.CombinedLoss(ignore_index=opt.ignore_index,weight=self.init_weight,sigmoid_normalization=opt.sigmoid_normalization)
            if (opt.loss_mode == 'GDL'):
                self.criterionagscSEVnet_final_output = losses.GeneralizedDiceLoss(ignore_index=opt.ignore_index,sigmoid_normalization=opt.sigmoid_normalization)

            if(opt.optim_mode == 'Adam'):
                self.optimizer_agscSEvnet = torch.optim.Adam(params=self.netagscSEVnet.parameters(),lr=opt.init_learningrate)
            if(opt.optim_mode == 'SGD'):
                self.optimizer_agscSEvnet = torch.optim.SGD(params=self.netagscSEVnet.parameters())
            self.optimizers.append(self.optimizer_agscSEvnet)
        #if use apex
        #self.netagscSEVnet,self.optimizer_vnet = amp.initialize(self.netagscSEVnet, self.optimizer_vnet, opt_level="O1")
    ...
    def forward(self):
        ...
        self.Prediction = self.netagscSEVnet(self.Image)  # netagscSEVnet(Image)
        ...
        if (self.loss_mode == 'CE'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    Mask += self.Mask[:, i,...] * i
            self.loss_agscSEVnet_final_output = self.criterionagscSEVnet_final_output(self.Prediction,Mask.long())
        if (self.loss_mode == 'DICE' or self.loss_mode == 'GDL'):
            self.loss_agscSEVnet_final_output = self.criterionagscSEVnet_final_output(self.Prediction, self.Mask)
        if (self.loss_mode == 'CBL'):
            for i in range(0,self.Mask.shape[1]):
                if(i==0):
                    ...
                    CEMask += self.Mask[:, i,...] * i
            self.loss_agscSEVnet_final_output = self.criterionagscSEVnet_final_output(self.Prediction, self.Mask, CEMask,self.flag,coeff_weight=self.coeff_weight)
        # if use apex
        # with amp.scale_loss(self.loss_agscSEVnet_final_output, self.optimizer_vnet) as scaled_loss:
            # scaled_loss.backward()
        self.loss_agscSEVnet_final_output.backward()

    def optimize_parameters(self):
        self.forward()
        self.optimizer_agscSEvnet.zero_grad()
        self.backward()
        self.optimizer_agscSEvnet.step()
